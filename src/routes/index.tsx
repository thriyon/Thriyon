...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "An invite-only operating system for the world's most ambitious creative practitioners." },
    ],
  }),
});
...
function SearchBar() {
  ...
  const suggestions = [
    "Brand Strategy", "3D & Motion", "Product Design", "Full-stack Engineering",
    "Type Design", "Photography", "Sound Design", "Art Direction",
  ];
  ...
  const handleSubmit = useCallback((e: React.FormEvent) => {
    e.preventDefault();
    if (query.trim()) navigate({ to: "/talent", search: {} });
  }, [query, navigate]);
  ...
  return (
    <form onSubmit={handleSubmit} className="relative w-full max-w-xl">
      <div className={`relative flex items-center rounded-full border transition-all duration-300 ${
        focused
          ? "border-accent/50 bg-white/8 shadow-[0_0_30px_-5px_oklch(0.7_0.18_295/0.3)]"
          : "border-white/12 bg-white/5"
      }`}>
        <svg className="ml-4 h-4 w-4 shrink-0 text-muted-foreground" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
        ...
        <input
          ...
          className="w-full bg-transparent px-3 py-3.5 text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none"
        />
        {query && (
          <button type="button" onClick={() => setQuery("")} className="mr-2 rounded-full p-1 text-muted-foreground hover:text-foreground">
      ...
      {focused && filtered.length > 0 && (
        <motion.div
          ...
          className="absolute left-0 right-0 top-full z-20 mt-2 overflow-hidden rounded-2xl border border-white/10 bg-card/95 backdrop-blur-xl"
        >
          {filtered.map((s) => (
            <button
              key={s}
              type="button"
              onMouseDown={() => { setQuery(s); navigate({ to: "/talent" }); }}
              className="flex w-full items-center gap-3 px-5 py-3 text-left text-sm text-muted-foreground transition hover:bg-white/5 hover:text-foreground"
            >
              <svg className="h-3.5 w-3.5 shrink-0 opacity-40" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
              </svg>
              {s}
            </button>
...
function Hero() {
  ...
  return (
    <section ref={ref} className="relative h-[100svh] min-h-[720px] w-full overflow-hidden">
        ...
        <div className="mt-10 grid items-end gap-8 md:grid-cols-[1fr_auto]">
          <div className="flex flex-col gap-6">
            ...
            >
              ...
              Cinematic. Sovereign. Quietly powerful.
            </motion.p>
            <motion.div
          ...
          >
            <Link to="/talent" className="group relative overflow-hidden rounded-full bg-white px-6 py-3 text-sm font-medium text-black transition hover:scale-[1.02]">
              Enter the index →
            </Link>
            <Link to="/how-it-works" className="rounded-full border border-white/15 px-6 py-3 text-sm text-foreground/90 transition hover:border-white/40 hover:bg-white/5">
              The method
            </Link>
          </motion.div>
        </div>
      ...
      <div className="absolute bottom-6 right-6 z-10 hidden font-mono text-[10px] uppercase tracking-[0.3em] text-muted-foreground md:block">
        ↓ Scroll · cinematic descent
      </div>
    </section>
  );
}
...
function Manifesto() {
  const lines = [
    "We believe the next decade",
    "of creative work belongs",
    "to the small, the sovereign,",
    "and the uncompromising.",
  ];
  return (
    <section className="mx-auto max-w-[1400px] px-6 py-40">
      <div className="mb-16 flex items-center gap-4 font-mono text-[11px] uppercase tracking-[0.3em] text-muted-foreground">
      ...
      <div className="font-display text-[clamp(2.2rem,6vw,5rem)] leading-[1.05] tracking-[-0.035em]">
        {lines.map((l, i) => (
          <motion.div
            ...
            className={i === lines.length - 1 ? "text-violet" : "text-gradient"}
          >
            {l}
          </motion.div>
        ))}
...
/* ─── Neural Network Orbit (categories) ─── */
...
const CONNECTIONS: [string, string][] = [
  ...
  ["brand", "product"], ["motion", "engineering"],
  ["sound", "code"],
  // Center connections
...
function ConnectionLine({ fromNode, toNode }: { fromNode: any, toNode: any }) {
  ...
  return (
    <motion.line
      x1={x1 as any} y1={y1 as any} x2={x2 as any} y2={y2 as any}
...
function NeuralPulse({ fromNode, toNode, delay }: { fromNode: any, toNode: any, delay: number }) {
  ...
  const cx = useTransform([fromNode.x, toNode.x, progress], ([fx, tx, p]: any[]) => `${fx + (tx - fx) * p}%`);
  const cy = useTransform([fromNode.y, toNode.y, progress], ([fy, ty, p]: any[]) => `${fy + (ty - fy) * p}%`);
  ...
  return (
    <motion.div
      ...
      onPointerDown={(e) => {
        ...
        const px = (node.x.get() / 100) * rect.width + rect.left;
        const py = (node.y.get() / 100) * rect.height + rect.top;
        dragOffset.current = { x: px - e.clientX, y: py - e.clientY };
      }}
      onPointerUp={(e) => {
        node.isDragging = false;
        e.currentTarget.releasePointerCapture(e.pointerId);
        const dist = Math.hypot(e.clientX - startPos.current.x, e.clientY - startPos.current.y);
        if (dist < 5) navigate({ to: "/talent" });
      }}
      onPointerMove={(e) => {
        ...
        node.x.set(Math.max(5, Math.min(95, ((targetX - rect.left) / rect.width) * 100)));
        node.y.set(Math.max(5, Math.min(95, ((targetY - rect.top) / rect.height) * 100)));
      }}
    >
      <div className={`group glass block rounded-2xl text-left hairline transition-all duration-300 hover:bg-white/10 hover:shadow-[0_0_20px_-5px_oklch(0.7_0.18_295/0.4)] ${sizeClasses[node.cat.size as keyof typeof sizeClasses]}`}>
        <div className="flex items-center gap-2.5">
          <div className={`grid place-items-center rounded-full bg-gradient-to-br from-accent/30 to-midnight border border-white/10 ${iconClasses[node.cat.size as keyof typeof iconClasses]}`}>
            <span className="text-accent">{node.cat.icon}</span>
          </div>
          <div className="pointer-events-none select-none">
            ...
            <div className="font-mono text-[8px] uppercase tracking-wider text-muted-foreground/70">
              {talents.filter((t) =>
                t.role.toLowerCase().includes(node.cat.label.split(" ")[0].toLowerCase()) ||
                t.skills.some((s) => s.toLowerCase().includes(node.cat.label.split(" ")[0].toLowerCase()))
              ).length || Math.floor(Math.random() * 20 + 5)} experts
            </div>
          </div>
        </div>
      </div>
...
function Orbit() {
  ...
  if (nodesRef.current.length === 0) {
    nodesRef.current = CATEGORIES.map((c) => ({
      cat: c,
      ox: c.x,
      oy: c.y,
      x: motionValue(c.x),
  ...
  useAnimationFrame(() => {
    nodesRef.current.forEach(node => {
      ...
      const springForceX = (node.ox - cx) * 0.0005;
      const springForceY = (node.oy - cy) * 0.0005;

      node.vx += springForceX + (Math.random() - 0.5) * 0.015;
      node.vy += springForceY + (Math.random() - 0.5) * 0.015;

      node.vx *= 0.98;
      node.vy *= 0.98;

      if (cx < 5) node.vx += 0.02;
      if (cx > 95) node.vx -= 0.02;
      if (cy < 5) node.vy += 0.02;
      if (cy > 95) node.vy -= 0.02;

      node.x.set(cx + node.vx);
      node.y.set(cy + node.vy);
    });
  });
    ...
    <section className="relative mx-auto max-w-[1400px] px-6 py-40">
      <div className="mb-12 grid gap-6 md:grid-cols-[1fr_auto] md:items-end">
        <div>
          ...
          <h2 className="font-display text-[clamp(2.2rem,5vw,4.5rem)] leading-[1] tracking-[-0.035em]">
            A neural network of <span className="text-violet">creative</span> disciplines.
          </h2>
        </div>
        <Link to="/talent" className="rounded-full border border-white/15 px-5 py-2.5 text-sm hover:bg-white/5">
          Explore categories →
        </Link>
      </div>
        ...
        <svg className="pointer-events-none absolute inset-0 h-full w-full" preserveAspectRatio="none">
          <defs>
            <linearGradient id="neural-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%" stopColor="oklch(0.7 0.18 295 / 0.4)" />
              <stop offset="50%" stopColor="oklch(0.7 0.18 295 / 0.15)" />
              <stop offset="100%" stopColor="oklch(0.7 0.18 295 / 0.4)" />
            </linearGradient>
            <filter id="glow">
          ...
          {CONNECTIONS.map(([a, b], i) => {
            ...
            return <ConnectionLine key={`conn-${a}-${b}`} fromNode={from} toNode={to} />;
          })}
          {CONNECTIONS.slice(0, 10).map(([a, b], i) => {
            const from = nodeMap[a];
            const to = nodeMap[b];
            if (!from || !to) return null;
            return <NeuralPulse key={`pulse-${a}-${b}`} fromNode={from} toNode={to} delay={i * 0.6} />;
          })}
        </svg>

        {nodesRef.current.map((node) => (
          <NetworkNode key={node.cat.id} node={node} containerRef={containerRef} />
        ))}
        {/* Center hub */}
        <div className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
          <div className="pulse-ring relative flex h-14 items-center justify-center rounded-full bg-white px-8 text-black">
            <span className="font-display text-sm font-medium tracking-[0.2em] uppercase">THRIYON</span>
          </div>
        </div>
      </div>
    </section>
...
function TalentTeaser() {
  return (
    <section className="mx-auto max-w-[1400px] px-6 py-40">
      <div className="mb-12 font-mono text-[11px] uppercase tracking-[0.3em] text-muted-foreground">
        III · Method
      </div>
      <div className="grid gap-8 md:grid-cols-3">
        {[
          ...
          <motion.div
            ...
            className="glass rounded-2xl p-7 hairline"
          >
    ...
    <section className="mx-auto max-w-[1400px] px-6 pt-32 pb-20">
      <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-b from-graphite/60 to-background p-10 md:p-20 grain glow">
        ...
        <div className="relative">
          <div className="mb-8 font-mono text-[11px] uppercase tracking-[0.3em] text-muted-foreground">
            IV · Enter
          </div>
          <h2 className="max-w-4xl font-display text-[clamp(2.6rem,8vw,7rem)] leading-[0.95] tracking-[-0.045em]">
            The future of creative work is <span className="text-violet">quietly</span> already here.
          </h2>
          <div className="mt-10 flex flex-wrap items-center gap-3">
            <Link to="/contact" className="rounded-full bg-white px-6 py-3 text-sm font-medium text-black">
              Request access →
            </Link>
            <Link to="/talent" className="rounded-full border border-white/15 px-6 py-3 text-sm hover:bg-white/5">
              Browse talent
            </Link>
          </div>
        </div>
