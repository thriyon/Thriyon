...
function SearchBar() {
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      e.preventDefault();
      if (query.trim()) router.push("/talent");
    },
    [query, router],
  );
  return (
    <form onSubmit={handleSubmit} className="relative w-full max-w-xl">
      <div
        className={`relative flex items-center rounded-full border transition-all duration-300 ${focused ? "border-accent/50 bg-white/8 shadow-[0_0_30px_-5px_oklch(0.7_0.18_295/0.3)]" : "border-white/12 bg-white/5"}`}
      >
        <svg
        ...
        {query && (
          <button
            type="button"
            onClick={() => setQuery("")}
            className="mr-2 rounded-full p-1 text-muted-foreground hover:text-foreground"
          >
            <svg
      ...
      <div className="relative z-10 mx-auto flex h-full max-w-[1400px] flex-col justify-end px-6 pb-24">
        ...
        <div className="mt-10 grid items-end gap-8 md:grid-cols-[1fr_auto]">
          <div className="flex flex-col gap-6">
            ...
            >
              An operating system for elite freelancers, studios and clients. Cinematic. Sovereign.
              Quietly powerful.
            </motion.p>
            <motion.div
          ...
          >
            <Link
              href="/talent"
              className="group relative overflow-hidden rounded-full bg-white px-6 py-3 text-sm font-medium text-black transition hover:scale-[1.02]"
            >
              Enter the index →
            </Link>
            <Link
              href="/how-it-works"
              className="rounded-full border border-white/15 px-6 py-3 text-sm text-foreground/90 transition hover:border-white/40 hover:bg-white/5"
            >
              The method
            </Link>
          </motion.div>
  ...
  return (
    <motion.line
      ...
      transition={{
        duration: 1.5,
        delay:
          (((fromNode?.cat?.id?.charCodeAt(0) || 0) + (toNode?.cat?.id?.charCodeAt(0) || 0)) % 15) *
          0.1,
      }}
    />
  );
...
function NeuralPulse({ fromNode, toNode, delay }: { fromNode: any; toNode: any; delay: number }) {
  ...
  const cx = useTransform(
    () => `${fromNode.x.get() + (toNode.x.get() - fromNode.x.get()) * progress.get()}%`,
  );
  const cy = useTransform(
    () => `${fromNode.y.get() + (toNode.y.get() - fromNode.y.get()) * progress.get()}%`,
  );
...
function NetworkNode({
  ...
  return (
    ...
    >
      <div
        className={`group glass block rounded-2xl text-left hairline transition-all duration-300 hover:bg-white/10 hover:shadow-[0_0_20px_-5px_oklch(0.7_0.18_295/0.4)] ${sizeClasses[node.cat.size as keyof typeof sizeClasses]}`}
      >
        <div className="flex items-center gap-2.5">
          <div
            className={`grid place-items-center rounded-full bg-gradient-to-br from-accent/30 to-midnight border border-white/10 ${iconClasses[node.cat.size as keyof typeof iconClasses]}`}
          >
            <span className="text-accent">{node.cat.icon}</span>
          </div>
          <div className="pointer-events-none select-none">
            <div
              className={`leading-tight font-medium ${textClasses[node.cat.size as keyof typeof textClasses]}`}
            >
              {node.cat.label}
            </div>
            <div className="font-mono text-[8px] uppercase tracking-wider text-muted-foreground/70">
              {talents.filter(
                (t) =>
                  ...
                  t.skills.some((s) =>
                    s.toLowerCase().includes(node.cat.label.split(" ")[0].toLowerCase()),
                  ),
              ).length || ((node.cat.id.charCodeAt(0) * 3) % 20) + 5}{" "}
              experts
            </div>
          </div>
        </div>
...
function Orbit() {
  ...
  if (!centerRef.current) {
    centerRef.current = { x: motionValue(50), y: motionValue(50) };
  }
  ...
  useAnimationFrame((t) => {
    ...
    const beat =
      (Math.pow(Math.sin(t * 0.002), 4) + Math.pow(Math.sin(t * 0.002 + 0.3), 4) * 0.5) * 2.5;
    ...
    for (let i = 0; i < n; i++) {
      ...
      const ax = nodeA.x.get();
      const ay = nodeA.y.get();
      ...
      for (let j = i + 1; j < n; j++) {
        ...
        const bx = nodeB.x.get();
        const by = nodeB.y.get();
        const dx = bx - ax;
        const dy = by - ay;
      ...
      const dxCenter = 50 - ax;
      const dyCenter = 50 - ay;
    ...
    nodesRef.current.forEach((node) => {
      ...
      // Bounding box bounce
      if (cx < 5) node.vx += 0.5;
      if (cx > 95) node.vx -= 0.5;
      if (cy < 5) node.vy += 0.5;
      if (cy > 95) node.vy -= 0.5;
  ...
  return (
    <section className="relative mx-auto max-w-[1400px] px-6 py-40">
      ...
      <div
        ref={containerRef}
        className="relative aspect-[16/10] min-h-[600px] w-full overflow-hidden rounded-3xl border border-white/8 bg-gradient-to-b from-graphite/40 to-background grain"
      >
        ...
        >
          ...
          {CONNECTIONS.slice(0, 10).map(([a, b], i) => {
            ...
            if (!from || !to) return null;
            return (
              <NeuralPulse key={`pulse-${a}-${b}`} fromNode={from} toNode={to} delay={i * 0.6} />
            );
          })}
        </svg>
        ...
        <div className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
          <div className="pulse-ring relative flex h-14 items-center justify-center rounded-full bg-white px-8 text-black">
            <span className="font-display text-sm font-medium tracking-[0.2em] uppercase">
              THRIYON
            </span>
          </div>
        </div>
        ...
        {[
          {
            n: "01",
            t: "Identify",
            b: "Curated talent across brand, product, motion, sound, type and code. Every profile, hand-reviewed.",
          },
          {
            n: "02",
            t: "Initiate",
            b: "Cinematic offers replace bid wars. Scoping, contracts, and milestones, in a single floating canvas.",
          },
          {
            n: "03",
            t: "Inhabit",
            b: "Work, message, deliver, get paid. A sovereign workspace tuned for slow, deep, long-arc creative practice.",
          },
        ].map((s, i) => (
          <motion.div
            key={s.n}
      ...
      <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-b from-graphite/60 to-background p-10 md:p-20 grain glow">
        ...
        <div className="relative">
          ...
          <h2 className="max-w-4xl font-display text-[clamp(2.6rem,8vw,7rem)] leading-[0.95] tracking-[-0.045em]">
            The future of creative work is <span className="text-violet">quietly</span> already
            here.
          </h2>
          <div className="mt-10 flex flex-wrap items-center gap-3">
            ...
            <Link
              href="/talent"
              className="rounded-full border border-white/15 px-6 py-3 text-sm hover:bg-white/5"
            >
              Browse talent
            </Link>
          </div>
